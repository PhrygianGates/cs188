    ...
    def elapseTime(self, gameState):
        ...
        for position in self.allPositions:
            ...
            for p in self.allPositions:
                self.beliefs[p] += old_beliefs[position] * newPosDist[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        repeat = self.numParticles // len(self.legalPositions)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            beliefs[p] += self.getObservationProb(observation, gameState.getPacmanPosition(), p, self.getJailPosition())
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        newPosDists = dict()
        for p in self.particles:
            if p not in newPosDists.keys():
                newPosDists[p] = self.getPositionDistribution(gameState, p)
            newParticles.append(newPosDists[p].sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        r = self.numParticles // (len(self.legalPositions) ** self.numGhosts)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(observation[i], pacmanPosition, p[i], self.getJailPosition(i))
            beliefs[p] += prob
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
            return
