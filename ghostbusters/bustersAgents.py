...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for beliefs in livingGhostPositionDistributions:
            positions.append(beliefs.argMax())
        position = min(positions, key = lambda x : self.distancer.getDistance(x, pacmanPosition))
        action = min(legal, key = lambda x : self.distancer.getDistance(position, Actions.getSuccessor(pacmanPosition, x)))
        return action
