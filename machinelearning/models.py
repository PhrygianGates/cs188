...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        xw0 = nn.ReLU((nn.AddBias(nn.Linear(x, self.w0), self.b0)))
        xw1 = nn.AddBias(nn.Linear(xw0, self.w1), self.b1)
        return xw1
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.w0.update(grad[0], self.rate)
                self.b0.update(grad[1], self.rate)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.w.update(grad[0], self.rate)
                self.wh.update(grad[1], self.rate)
                self.wf.update(grad[2], self.rate)
